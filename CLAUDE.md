...
| `hand_evaluator.py` | Hand strength evaluation (re-exports from `ml/utils`) |
| `personality.py` | Random personality generation for Hard AI |
| `opponent_model.py` | Void detection + opponent needs tracking |
...
| File | Responsibility |
|------|---------------|
| `constants.py` | `DIMENSIONS` tuple (11 dims — single source of truth) |
| `utils.py` | `clamp01()`, `HandEvaluation`, `evaluate_hand()` |
| `data_store.py` | `JsonlFileStore` — JSONL read/write abstraction |
| `learning/neighbor_model.py` | kNN algorithm (uses DataStore) |
| `learning/features.py` | Feature extraction (hand → float vector) |
| `learning/decision_collector.py` | Buffer decisions, flush winners to JSONL |
| `analysis/fingerprint.py` | Player trait vector (11 dims, exponential decay) |
| `analysis/persona_match.py` | Weighted multi-factor persona scoring |
| `analysis/persona_loader.py` | Persona class + JSON corpus loader |
| `analysis/personas.json` | 75 personas across 7 categories |
...
| `update.py` | In-app update: `/api/update/check`, `/api/update/apply`, `/api/update/status` |
| `data_sharing.py` | Community data: `/api/data/share`, `/api/data/community/download` |
...
- **Must-lose mode (turbulence):** Only the dealer (last bidder) is constrained — same rule as standard mode
...
### Smoke tests (this repo — 143 tests)

```
backend/tests/                                          # 122 backend smoke tests
├── test_trick_resolver.py          #   6 tests — trick winner logic
├── test_scorer.py                  #   7 tests — scoring formula
├── test_validators.py              #  18 tests — bid constraints, follow-suit, forbidden bids
├── test_engine.py                  #  17 tests — full game flow, state transitions
├── test_engine_correctness.py      #  18 tests — additional state machine coverage
├── test_api_rest.py                #  40 tests — REST endpoints, lobby, quick-join, AI auto-play
├── test_round_config.py            #  12 tests — JSON config loading, bridge tests vs runtime
├── test_websocket_game.py          #  13 tests — WS connect, two-round play, stuck detect, reconnect
└── test_deck.py                    #   9 tests — deck creation, shuffle, deal

frontend/src/                                           #  21 frontend smoke tests
└── hooks/useGame.test.ts           #  21 tests — reducer: all event types, buffering, replay
```

### Full test suite ([jvalin17/judgement-tests](https://github.com/jvalin17/judgement-tests) — 484 tests)

Runs on every push via GitHub Actions. Clone as sibling and run `./run-test-suite.sh`.

```
backend/   (320 tests)  — AI strategies, analysis/persona, round manager,
                          card play helpers, game manager, smart AI, edge cases,
                          info isolation, multiplayer integration
frontend/  (164 tests)  — all UI components, type helpers, hooks
```

## Common Tasks

**Add a new AI difficulty:** Create `backend/app/ai/mythical.py` implementing `AIStrategy` with `strategy_type = "mythical"`, use shared helpers from `card_play.py`, add to `game_manager.STRATEGY_MAP`, add enum value to `models/player.py:AIDifficulty`.
...
**Before every push**, run through this list:

1. **Frontend tests** — `cd frontend && npx vitest run` (all must pass; add/update tests for changed components)
2. **Backend tests** — `python3 -m pytest backend/tests/ -v` (all must pass; new features need tests)
3. **Pydantic** — `python3 -c "from pydantic import BaseModel"` (verify after any pip install)
4. **TypeScript** — `cd frontend && npx tsc -b` (no type errors)
5. **README** — update if change affects features, test counts, download links, variants, or documented behavior. Verify links work (e.g. download URLs match actual release artifact names — check with `gh release view --json assets`)
6. **RELEASE_NOTES.md** — add entry under Unreleased (or current version if tagging)
7. **LEARNINGS.md** — if something was missed or caught late, add an entry so the same mistake isn't repeated
