...
class TestHardAITrumpManagement:
    ...
    def test_lowest_winning_trump_in_trick(self):
        """When trumping in, use lowest trump that wins."""
        from backend.app.ai.card_play import lowest_winning_trump
        valid_cards = [
            Card(suit=Suit.SPADES, rank=Rank.ACE),
            Card(suit=Suit.SPADES, rank=Rank.THREE),
        ]
        trick_cards = [Card(suit=Suit.HEARTS, rank=Rank.KING)]
        result = lowest_winning_trump(valid_cards, trick_cards, Suit.SPADES)
        assert result is not None
        assert result.rank == Rank.THREE  # cheapest trump
